...
def detect_from_camera(cam_index, barrier):
    ...
    print(f"📷 Camera {cam_index} initialized, waiting to start...")
    barrier.wait()  # Synchronize all threads
    print(f"✅ Camera {cam_index} started monitoring")
    log_event(f"✅ Camera {cam_index} started monitoring")
...
camera_indices = [0]  # Add your camera indices here
start_barrier = threading.Barrier(len(camera_indices))  # Synchronization barrier

# Start all camera threads
threads = []
for index in camera_indices:
    t = threading.Thread(target=detect_from_camera, args=(index, start_barrier))
    t.start()
    threads.append(t)
